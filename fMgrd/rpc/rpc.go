...
func getClient(logger *logging.Writer, fileName string, process string) (*ClientJson, error) {
	...
	if err != nil {
		logger.Err(fmt.Sprintf("Failed to open FMGRD config file:%s, err:%s", fileName, err))
		return nil, err
	}
...
func StartServer(logger *logging.Writer, handler *FMGRHandler, paramDir string) {
	fileName := paramDir
	...
	clientJson, err := getClient(logger, fileName, "fMgrd")
